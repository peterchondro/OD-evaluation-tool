...
class AnalysisThread_2D(QThread):
	...
	def run(self):
		...
		self.update.emit(GT, TP, FP, FN, self.lACC, self.lFPR, self.lFNR, self.gACC, -1, self.gFPR, -1, self.gFNR, -1, self.ERHist)
		self.end.emit()
...
class AnalysisDataSetThread_2D(QThread):
	...
	def __init__(self, GT_path, Result_path, header_class, weighted_parameters):
		...
		self.gSum = 0.0
		self.Accuracy_Text = ""
		self.nCONFUSE = np.zeros((self.class_num, self.class_num), dtype=np.int64)
		self.nNOTDETECT = np.zeros(self.class_num, dtype=np.int64)
		self.nOTHER = np.zeros(self.class_num, dtype=np.int64)
			...
			if(loneCounter == n_rsBB):
				...
				if(IOU_score > IOU_th):
					...
					if(classRS == -1):
						...
						if(classGT == classRS):
							self.TP[classGT]+=1
							self.nCONFUSE[classGT][classRS]+=1
						else:
							self.FN[classGT]+=1
							self.FP[classRS]+=1
	...
	def _setresulttext(self):
		...
		print("CONFUSION MATRIX:", self.nCONFUSE)
		print("NOT DETECTED:", self.nNOTDETECT)
		print("OTHERS:", self.nOTHER)
...
class ToolUI(qws.QWidget):
	...
	def _AnalysisThreadUpdate(self, GT, TP, FP, FN, lACC, lFPR, lFNR, gACC, wgACC, gFPR, wgFPR, gFNR, wgFNR, ERHist):
		...
		for k in range(self.class_num):
			if (self.lFNR[k] == -1):
				...
				lRCL = 1 - self.lFNR[k]
			print(self.lACC[k], '    ', 1 - self.lFPR[k], '    ', lRCL)
		print("\n")
	...
	def _SetResultText(self):
		...
		if (self.gFNR == -1):
			...
			gRCL = 1 - self.gFNR
		print(self.gACC, '   ', self.wgACC, '   ', 1 - self.gFPR, '   ', 1 - self.wgFPR, '   ', gRCL, '   ', 1 - self.wgFNR)
		print("\n")
