...
class AnalysisThread_2D(QThread):
	update = pyqtSignal(object, object, object, object, object, object, object, object, object, object, object, object, object)
	end = pyqtSignal()
	...
	def run(self):
		...
		GT, TP, FP, FN, ER, nFP_matrix = self.BB2DEvaluation(gtDet, n_gtDet, rsDet, n_rsDet)
		max_error = 0
		best_error = 0
		...
		self.update.emit(GT, TP, FP, FN, nFP_matrix, self.lACC, self.lFPR, self.lFNR, self.gACC, 0, self.gFPR, self.gFNR, self.ERHist)
		self.end.emit()
	...
	def BB2DEvaluation(self, gtBB, n_gtBB, rsBB, n_rsBB):
		...
		nFP_matrix = np.zeros((self.class_num+1,self.class_num+1) , dtype=np.int64)
		x_res = 0.01
		y_res = 0.01
		IOU_th = 0.5
		for n in range(n_gtBB):
			...
			IOU_score = -1
			x_startGT = gtBB[n][0]
			x_stopGT = gtBB[n][0] + gtBB[n][2]
			y_startGT = gtBB[n][1]
			y_stopGT = gtBB[n][1] + gtBB[n][3]
			x_cntGT = (x_stopGT + x_startGT) / 2
			y_cntGT = (y_stopGT + y_startGT) / 2
			classGT = gtBB[n][4]
			nGT[classGT]+=1
			if(gtBB[n][2] > gtBB[n][3]):
				th_dt = gtBB[n][2]
			else:
				th_dt = gtBB[n][3]
			for r in range(n_rsBB):
				x_startRS = rsBB[r][0]
				x_stopRS = rsBB[r][0] + rsBB[r][2]
				y_startRS = rsBB[r][1]
				y_stopRS = rsBB[r][1] + rsBB[r][3]
						...
						for x in np.arange(x_startGT, x_stopGT):
							if(x > x_startRS and x < x_stopRS):
								if(y > y_startRS and y < y_stopRS):
									intersection+=1
					inArea = intersection
					gtArea = gtBB[n][2] * gtBB[n][3]
					rsArea = rsBB[r][2] * rsBB[r][3]
			...
			if(loneCounter == n_rsBB):
				...
				if(IOU_score > IOU_th):
					...
					nFN[classGT]+=1
		for n in range(n_rsBB):
###
			loneCounter_M = 0
			IOU_score_M = -1
			x_startRS = rsBB[n][0]
			x_stopRS = rsBB[n][0] + rsBB[n][2]
			y_startRS = rsBB[n][1]
			y_stopRS = rsBB[n][1] + rsBB[n][3]
			x_cntRS = (x_stopRS + x_startRS) / 2
			y_cntRS = (y_stopRS + y_startRS) / 2
			classRS_M = rsBB[n][4]

			if(rsBB[n][2] > rsBB[n][3]):
				th_dt = rsBB[n][2]
			else:
				th_dt = rsBB[n][3]
			for r in range(n_gtBB):
				x_startGT = gtBB[r][0]
				x_stopGT = gtBB[r][0] + gtBB[r][2]
				y_startGT = gtBB[r][1]
				y_stopGT = gtBB[r][1] + gtBB[r][3]
				x_cntGT = (x_stopGT + x_startGT) / 2
				y_cntGT = (y_stopGT + y_startGT) / 2
				classGT_M = gtBB[r][4]
				x_aux = x_cntGT - x_cntGT
				y_aux = y_cntGT - y_cntGT
				xy_dt = math.sqrt((x_aux * x_aux) + (y_aux * y_aux))
				if(xy_dt < th_dt):
					intersection = 0
					for y in np.arange(y_startRS, y_stopRS):
						for x in np.arange(x_startRS, x_stopRS):
							if(x > x_startGT and x < x_stopGT):
								if(y > y_startGT and y < y_stopGT):
									intersection+=1
					inArea = intersection
					rsArea = rsBB[n][2] * rsBB[n][3]######
					gtArea = gtBB[r][2] * gtBB[r][3]
					if(inArea > gtArea or inArea > rsArea):
						if(gtArea > rsArea):
							inArea = rsArea
						else:
					...
					unArea = gtArea + rsArea - inArea
					score = inArea / unArea
					if(IOU_score_M < score):
						IOU_score_M = score
						pairOptimum_M = r
				else:
					loneCounter_M+=1
			if(loneCounter_M == n_gtBB):
				print("nothing")						
			else:
				if(IOU_score_M > IOU_th):
					classGT_M = gtBB[pairOptimum_M][4]
					if(classGT_M != -1):
						if(classRS_M != classGT_M):
							nFP_matrix[classGT_M][classRS_M]+=1
###
			if(pairStatusRS[n] == False):
				classRS = int(rsBB[n][4])
				if(classRS == -1):
					nFP[self.class_num]+=1
				else:
					nFP[classRS]+=1
		return nGT, nTP, nFP, nFN, hER, nFP_matrix
...
class AnalysisDataSetThread_2D(QThread):
	...
	def __init__(self, GT_path, Result_path, header_class, weighted_parameters):
		...
		self.nFP_matrix = np.zeros((self.class_num+1,self.class_num+1) , dtype=np.int64)
		self.weighted_parameters =  weighted_parameters
		self.lACC = [0 for x in range(self.class_num)]
		self.lFPR = [0 for x in range(self.class_num)]
		self.lFNR = [0 for x in range(self.class_num)]
	...
	def _analysis(self, gt, result):
		self.file_gt = gt
		self.file_result = result
		gtDet, n_gtDet = self.readDet_2DGT()
		rsDet, n_rsDet = self.readDet_2DRS()
		self.BB2DEvaluation(gtDet, n_gtDet, rsDet, n_rsDet)
	...
	def BB2DEvaluation(self, gtBB, n_gtBB, rsBB, n_rsBB):
		...
		nFN = np.zeros(self.class_num , dtype=np.int64)
		hER = np.zeros((self.class_num , self.class_num ), dtype=np.int64)
		nFP_matrix = np.zeros((self.class_num,self.class_num) , dtype=np.int64)
		x_res = 0.01
		y_res = 0.01
		IOU_th = 0.5
		for n in range(n_gtBB):
...
###
			...
			if(loneCounter_M == n_gtBB):
				...
				if(IOU_score_M > IOU_th):
					#print('IOU_score_M:', IOU_score_M)
					classGT_M = gtBB[pairOptimum_M][4]					
					if(classGT_M != -1):   #Jessie add for FP analizing
						if(classRS_M != classGT_M):
							# print('-----------------')
							# print('classGT_M:', classGT_M)
							# print('classRS_M:', classRS_M)
							self.nFP_matrix[classGT_M][classRS_M]+=1
###			
			if(pairStatusRS[n] == False):
				classRS = int(rsBB[n][4])
				if(classRS == -1):
					self.FP[self.class_num]+=1
					#self.nFP_matrix[classGT][self.class_num]+=1
				else:
					self.FP[classRS]+=1
					#self.nFP_matrix[classGT][classRS]+=1
	...
	def run(self):
		...
		for idx, filenames in enumerate(zip(gt_filenames, result_filenames)):
			...
			self._analysis(os.path.join(self.GT_path, filenames[0]), os.path.join(self.Result_path, filenames[1]))
		self._setresulttext()
		self.update.emit(self.GT, self.TP, self.FP, self.FN, self.nFP_matrix, self.lACC, self.lFPR, self.lFNR, self.gACC,self.wgACC, self.gFPR, self.gFNR, self.ERHist)
		self.end.emit(True)
...
class ToolUI(qws.QWidget):
	...
	def Change_File(self):
		...
		if(s.currentItem() == None):
			...
			if(os.path.splitext(self.Weights_Label.text())[1] == ".txt"):
				...
				self.weighted_parameters = [int(line.replace("\n","")) for line in lines]
	...
	def _AnalysisThreadUpdate(self, GT, TP, FP, FN, nFP_matrix,lACC, lFPR, lFNR, gACC, wgACC, gFPR, gFNR, ERHist):
		self.GT = GT
		self.TP = TP
		self.FP = FP
		self.FN = FN
		self.nFP_matrix = nFP_matrix
		self.lACC = lACC
		self.lFPR = lFPR
		self.lFNR = lFNR
		self.gACC = gACC
		...
		for k in range(self.class_num):
			...
			print("\n")
		print("FP Analysis:")
		print("Person - Rider - 2w - 4w - signBox - red - yellow - green - go_straight - turn_right - turn_left")
		for k in range(self.class_num):
			print(self.nFP_matrix[k][0], '   ', self.nFP_matrix[k][1], '   ', self.nFP_matrix[k][2], '   ', self.nFP_matrix[k][3], '   ', self.nFP_matrix[k][4], '  ', self.nFP_matrix[k][5], '   ', self.nFP_matrix[k][6], '   ', self.nFP_matrix[k][7], '   ', self.nFP_matrix[k][8], '   ', self.nFP_matrix[k][9], '   ', self.nFP_matrix[k][10])
			print("\n")
